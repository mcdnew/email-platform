...
def show():
    ...
    # top‐level view chooser
    view = st.radio(
        "View:",
        ["🆕 New Prospects", "📋 Active Prospects"],
        horizontal=True,
        index=0,
        key="prospects_view"
    )
    ...
    if view == "🆕 New Prospects":
        ...
        if submitted:
            if not (n and e):
                ...
                try:
                    r = requests.post(
                        f"{API_URL}/prospects",
                        json={"name": n, "email": e, "title": t, "company": c}
                    )
                    r.raise_for_status()
        ...
        if df_un.empty:
            ...
            pre_rows = [r for r in unassigned if r["id"] in prev_ids]

            gb = GridOptionsBuilder.from_dataframe(df_un)
            gb.configure_side_bar()  # enable column chooser + filters
            gb.configure_selection("multiple", use_checkbox=True, pre_selected_rows=pre_rows)
            gb.configure_pagination(paginationAutoPageSize=False,
                                    paginationPageSize=st.session_state.get("new_page_size", 20))
            grid = AgGrid(
                df_un,
            ...
            if sel_all:
                selected = df_un.to_dict("records")
            ...
            with c1:
                ...
                start    = st.date_input("First e-mail date", value=_dt.date.today())
            ...
            if assign:
                ...
                try:
                    r2 = requests.post(
                        f"{API_URL}/assign-sequence",
                        json={
                            "prospect_ids": ids,
                            "sequence_id": name_to_id[seq_pick],
                            "ventilate_days": vent,
    ...
    # ─── Active Prospects ───────────────────────────────────────────────────────
        ...
        if not active:
            st.info("No active prospects yet.")
            return

        # enrich with steps/textual progress
        for p in active:
            ...
            p["status"]        = _pretty_status(
                "completed" if done == total and total > 0 else "in_progress"
            )
        ...
        for col in ["sequence_steps_total","sequence_step_current","sequence_progress_pct"]:
            if col in df_act.columns:
                df_act = df_act.drop(columns=[col])
        ...
        gb2.configure_selection("multiple", use_checkbox=True, pre_selected_rows=pre2)
        ...
        sel2  = _extract(grid2, "selected_rows")
        edit2 = _extract(grid2, "data")
            ...
            try:
                ...
                for s in tl:
                    st.sidebar.markdown(
                        ...
                        f"Sent: {s['sent_at'] or '-'}  \n"
                        f"<span style='background:{colors.get(s['status'],'#bbb')};"
                        f"color:white;padding:2px 4px;border-radius:4px;'>"
                        f"{_pretty_status(s['status'])}</span>  \n"
                        f"Opened: {s['opened_at'] or '-'}",
                        unsafe_allow_html=True
                    )
            except:
                st.sidebar.error("Timeline load failed.")

        if sel2:
            st.divider()
            st.markdown(f"**Bulk actions ({len(sel2)})**")
                ...
                if st.button("Re-assign"):
                    try:
                        r3 = requests.post(
                            f"{API_URL}/assign-sequence",
                            json={
                                "prospect_ids": [x["id"] for x in sel2],
                                "sequence_id": name_to_id[pick2],
                                "ventilate_days": 0,
                            },
                        )
                        r3.raise_for_status()
                        st.success("Re-assigned ✔")
                        st.cache_data.clear()
                        try:
                            st.experimental_rerun()
                        except AttributeError:
            ...
            with b2:
                if st.button("Clear sequence"):
                    for x in sel2:
                        requests.put(f"{API_URL}/prospects/{x['id']}", json={"sequence_id": None})
                    st.success("Cleared ✔")
                    st.cache_data.clear()
                    try:
                        st.experimental_rerun()
            ...
            with b3:
                if st.button("❌ Delete"):
                    for x in sel2:
                        requests.delete(f"{API_URL}/prospects/{x['id']}")
                    st.warning("Deleted ✔")
                    st.cache_data.clear()
                    try:
                        st.experimental_rerun()
